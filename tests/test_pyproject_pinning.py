...
def test_no_main_branch_in_git_deps():
    ...
    assert main_matches == [], (
        f"Found @main reference(s) in git dependencies: {main_matches}. Replace with a full commit SHA."
    )
...
def test_basicsr_pinned_to_commit_hash():
    ...
    assert _SHA_RE.search(line), f"basicsr is not pinned to a commit hash. Found: {line.strip()}"
...
def test_gfpgan_pinned_to_commit_hash():
    ...
    assert _SHA_RE.search(line), f"gfpgan is not pinned to a commit hash. Found: {line.strip()}"
