...
_WEBCAM_FILE = (
    pathlib.Path(__file__).parent.parent / "modules" / "ui_webcam.py"
)
...
class TestProcessorCaptureBeforeLoop:
    def test_get_frame_processors_modules_called_before_while_loop(self):
        ...
        assert call_lines, (
            "get_frame_processors_modules is never called in _processing_thread_func"
        )
    ...
    def test_get_frame_processors_modules_called_exactly_once(self):
        ...
        assert len(call_lines) == 1, (
            f"Expected exactly 1 call to get_frame_processors_modules, "
            f"found {len(call_lines)} at lines {call_lines}."
        )
