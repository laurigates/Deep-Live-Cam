...
def parse_args() -> None:
    ...
    program.add_argument(
        "--png-frames",
        help="use lossless PNG for intermediate frames (larger files, no artifacts)",
        dest="use_png_frames",
        action="store_true",
        default=False,
    )
    ...
    program.add_argument(
        "--keep-audio", help="keep original audio", dest="keep_audio", action="store_true", default=True
    )
    program.add_argument(
        "--keep-frames", help="keep temporary frames", dest="keep_frames", action="store_true", default=False
    )
    program.add_argument(
        "--many-faces", help="process every face", dest="many_faces", action="store_true", default=False
    )
    program.add_argument(
        "--nsfw-filter", help="filter the NSFW image or video", dest="nsfw_filter", action="store_true", default=False
    )
    program.add_argument(
        "--map-faces", help="map source target faces", dest="map_faces", action="store_true", default=False
    )
    program.add_argument(
        "--mouth-mask", help="mask the mouth region", dest="mouth_mask", action="store_true", default=False
    )
    program.add_argument(
        "--color-correction",
        help="color correction mode for the swapped face crop (none=off, lab=LAB mean/std transfer, histogram=per-channel CDF matching)",
        dest="color_correction_mode",
        default="none",
        choices=["none", "lab", "histogram"],
    )
    program.add_argument(
        "--occlusion-mask",
        help="preserve occluding objects (hands, glasses) during face swap",
        dest="occlusion_mask",
        action="store_true",
        default=False,
    )
    ...
    program.add_argument(
        "--virtual-cam", help="output to virtual camera device", dest="virtual_cam", action="store_true", default=False
    )
    program.add_argument(
        "--rife",
        help="enable RIFE frame interpolation for video output",
        dest="rife_enabled",
        action="store_true",
        default=False,
    )
    program.add_argument(
        ...
        dest="rife_model",
        default="rife-v4.25-lite",
        choices=["rife-v4.25", "rife-v4.25-lite"],
    )
    program.add_argument(
        "--rife-multiplier",
        help="RIFE frame rate multiplier (2=double, 4=quadruple)",
        dest="rife_multiplier",
        type=int,
        default=2,
        choices=[2, 4],
    )
    program.add_argument(
        "--half-rate",
        help="enable half-rate face processing with RIFE interpolation for live mode",
        dest="half_rate_processing",
        action="store_true",
        default=False,
    )
    program.add_argument(
        "--keyframe-interval",
        help="process every Nth frame in half-rate mode (2-10)",
    ...
    program.add_argument(
        "--live-enhance-size",
        help="face alignment resolution for enhancement in live mode; smaller values reduce warp/paste cost (default: 256)",
        dest="live_enhance_size",
        type=int,
        default=256,
    ...
    program.add_argument(
        "--landmark-smoothing",
        help="apply EMA smoothing to face bounding boxes and keypoints in live mode to reduce jitter",
        dest="landmark_smoothing",
        action="store_true",
        default=False,
    )
    program.add_argument(
        "--landmark-smoothing-alpha",
        help="EMA alpha for landmark smoothing: weight given to current frame (0.0=full history, 1.0=no smoothing, default: 0.7)",
        dest="landmark_smoothing_alpha",
        type=float,
        default=0.7,
    )
    program.add_argument(
        "--face-swap-model",
        help="face swap model variant to use (default: inswapper)",
    ...
    program.add_argument(
        "--max-memory", help="maximum amount of RAM in GB", dest="max_memory", type=int, default=suggest_max_memory()
    )
    program.add_argument(
...
def decode_execution_providers(execution_providers: list[str]) -> list[str]:
    return [
        ...
        for provider, encoded_execution_provider in zip(
            _AVAILABLE_PROVIDERS, encode_execution_providers(_AVAILABLE_PROVIDERS)
        )
...
def limit_resources(config: ProcessingConfig | None = None) -> None:
    if config is None:
...
def release_resources(config: ProcessingConfig | None = None) -> None:
    if config is None:
        config = build_config_from_globals()
    if (
...
def start(config: ProcessingConfig | None = None) -> None:
...
def _run_processor_pre_checks(config: ProcessingConfig | None = None) -> None:
