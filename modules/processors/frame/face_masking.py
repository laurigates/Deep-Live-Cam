...
def apply_histogram_matching(source: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    for ch in range(3):
        ...
        src_vals, src_inv, src_cnt = np.unique(src_ch.ravel(), return_inverse=True, return_counts=True)
        ...
        result_channels.append(mapped[src_inv].reshape(src_ch.shape).clip(0, 255).astype(np.uint8))
