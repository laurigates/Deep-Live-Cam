...
def find_tcl():
    hits = glob.glob(os.path.join(sys.base_prefix, "lib", "tcl*", "init.tcl")) + glob.glob(
...
def find_tk():
    hits = glob.glob(os.path.join(sys.base_prefix, "lib", "tk[0-9]*")) + glob.glob(
        os.path.join(sys.base_prefix, "tcl", "tk[0-9]*")
    )
